...
# KOKORO_ROOT is needed by `kokoro/testutils/install_tink_via_pip.sh`
...
if [[ -n "${TINK_REMOTE_BAZEL_CACHE_GCS_BUCKET:-}" ]]; then
  ...
  TEST_SCRIPT_ARGS+=(
    -c "${TINK_REMOTE_BAZEL_CACHE_GCS_BUCKET}/bazel/${TINK_PY_BASE_IMAGE_HASH}"
  )
fi
readonly TEST_SCRIPT_ARGS

readonly TEST_SCRIPT="kokoro/gcp_ubuntu/pip/test_script.sh"
