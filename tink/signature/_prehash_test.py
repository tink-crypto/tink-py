...
def _create_prehasher(
  ...
  return _prehash.PrehashCcToPyWrapper(
      tink_bindings.create_prehash(keyset.SerializeToString())
  )
...
class PrehashTest(parameterized.TestCase):
  ...
  def test_prehash_success(self, template):
    ...
    data = b'message to be prehashed'
    prehash = prehasher.compute(data)
    self.assertNotEmpty(prehash)
  ...
  def test_prehash_matches_expected(
      self, instance, public_key_bytes, expected_prehash
  ):
    prehasher = _create_prehasher(instance, public_key_bytes, _KEY_ID)
    prehash = prehasher.compute(_TEST_MESSAGE)
    self.assertEqual(prehash, expected_prehash)

  def test_prehash_unsupported_key_type_fails(self):
    ...
    with self.assertRaises(tink_bindings.PythonTinkException):
      _ = _prehash.PrehashCcToPyWrapper(
          tink_bindings.create_prehash(
