...
"""Tests for SignPrehash primitive."""
...
_KEY_ID = 0x01020304
_EXPECTED_PREFIX = b'\xff\x01\x02\x03\x04'

# Test values from
# third_party/openssl/boringssl/src/third_party/wycheproof_testvectors/mldsa_{44,65,87}_sign_seed_test.json.
_PRIVATE_SEED = bytes.fromhex('2a' * 32)

_ML_DSA_44_PUBLIC_KEY = bytes.fromhex(
...
_ML_DSA_65_PUBLIC_KEY = bytes.fromhex(
...
_ML_DSA_65_EXPECTED_MU = bytes.fromhex(
...
_ML_DSA_87_PUBLIC_KEY = bytes.fromhex(
...
_ML_DSA_87_EXPECTED_MU = bytes.fromhex(
...
_TEST_MESSAGE = bytes.fromhex('48656c6c6f20776f726c64')


def _create_sign_prehasher(
    instance: ml_dsa_pb2.MlDsaInstance,
    public_key_bytes: bytes,
    private_seed: bytes,
    key_id: int,
) -> _sign_prehash.SignPrehash:
  public_key_proto = ml_dsa_pb2.MlDsaPublicKey(
      version=0,
      params=ml_dsa_pb2.MlDsaParams(ml_dsa_instance=instance),
      key_value=public_key_bytes,
  )
  private_key_proto = ml_dsa_pb2.MlDsaPrivateKey(
      version=0,
      key_value=private_seed,
      ...
      key=[
          tink_pb2.Keyset.Key(
              key_data=tink_pb2.KeyData(
                  ...
                  value=private_key_proto.SerializeToString(),
  ...
  return _sign_prehash.SignPrehashCcToPyWrapper(
      tink_bindings.create_sign_prehash(keyset.SerializeToString())
  )
...
def _create_verifier(
      ...
      key=[
          tink_pb2.Keyset.Key(
              key_data=tink_pb2.KeyData(
                  ...
                  value=public_key_proto.SerializeToString(),
  ...
  return handle.primitive(signature.PublicKeyVerify)


class SignPrehashTest(parameterized.TestCase):

  @parameterized.parameters([
      signature.signature_key_templates.ML_DSA_44,
      signature.signature_key_templates.ML_DSA_65,
      signature.signature_key_templates.ML_DSA_87,
  ])
  def test_sign_prehash_success(self, template):
    private_handle = tink.new_keyset_handle(template)
    public_handle = private_handle.public_keyset_handle()

    prehasher = _prehash.PrehashCcToPyWrapper(
        tink_bindings.create_prehash(public_handle._keyset.SerializeToString())
    )
    signer = _sign_prehash.SignPrehashCcToPyWrapper(
        tink_bindings.create_sign_prehash(
            private_handle._keyset.SerializeToString()
        )
    )
  ...
  @parameterized.parameters([
      (
          ml_dsa_pb2.MlDsaInstance.ML_DSA_44,
          _ML_DSA_44_PUBLIC_KEY,
          _EXPECTED_PREFIX + _ML_DSA_44_EXPECTED_MU,
      ),
      (
          ml_dsa_pb2.MlDsaInstance.ML_DSA_65,
          _ML_DSA_65_PUBLIC_KEY,
          _EXPECTED_PREFIX + _ML_DSA_65_EXPECTED_MU,
      ),
      (
          ml_dsa_pb2.MlDsaInstance.ML_DSA_87,
          _ML_DSA_87_PUBLIC_KEY,
          _EXPECTED_PREFIX + _ML_DSA_87_EXPECTED_MU,
      ),
  ])
  def test_sign_prehash_matches_expected(
      self, instance, public_key_bytes, expected_prehash
  ):
    signer = _create_sign_prehasher(
        instance, public_key_bytes, _PRIVATE_SEED, _KEY_ID
    )
    sig = signer.sign(expected_prehash)
    verifier = _create_verifier(instance, public_key_bytes, _KEY_ID)
    verifier.verify(sig, _TEST_MESSAGE)
    with self.assertRaises(tink.TinkError):
      verifier.verify(sig, b'wrong message')

  def test_sign_prehash_unsupported_key_type_fails(self):
    ...
    with self.assertRaises(tink_bindings.PythonTinkException):
      _ = _sign_prehash.SignPrehashCcToPyWrapper(
          tink_bindings.create_sign_prehash(
              private_handle._keyset.SerializeToString()
          )
      )

  def test_sign_prehash_on_public_keyset_fails(self):
    private_handle = tink.new_keyset_handle(
        signature.signature_key_templates.ML_DSA_65
    )
    public_handle = private_handle.public_keyset_handle()
    ...
    with self.assertRaises(tink_bindings.PythonTinkException):
      _ = _sign_prehash.SignPrehashCcToPyWrapper(
          tink_bindings.create_sign_prehash(
              public_handle._keyset.SerializeToString()
          )
      )
