...
class HybridKeyManagerTest(parameterized.TestCase):
  ...
  def test_new_key_data(self):
    ...
    self.assertEqual(
        key.public_key.params.kem_params.curve_type, common_pb2.NIST_P256
    )
...
class HpkeKeyManagerTest(parameterized.TestCase):
  ...
  def test_new_key_data(self):
    ...
    self.assertEqual(
        key.public_key.params.kem, hpke_pb2.DHKEM_X25519_HKDF_SHA256
    )
  ...
  @parameterized.parameters([
      ...
      hybrid.hybrid_key_templates.X_WING_HKDF_SHA256_HKDF_SHA256_AES_256_GCM_RAW,
  ])
  def test_encrypt_decrypt(self, template):
    private_handle = tink.new_keyset_handle(template)
    public_handle = private_handle.public_keyset_handle()
